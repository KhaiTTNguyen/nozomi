...
class TestDiffSim3D(unittest.TestCase):
    ...
    #     ## CREATE DIRECTORY 
    #     spin_folder_name = os.path.join('.','diffsim3d','animation','debug_spin', 'ADC', 'intra_axonal_JAN15', str(date_time)+'_R'+str(sr[-1])+'_undu_freq'+str(undu_freq)+'_undu_amp'+str(undu_amp)+'_dt_'+str(dt)+'_Lz_'+str(Lz))
    ...
    def testInsideTwoSetOfMultipleSpheresSINE(self):
        '''
        ...
        #     diff_time[n] = n*dt
        # #-------------------------------------------
        # # after simulating diffusion, are all spins still inside axons
        # isInside = False
        # for struct in sg3.structures[:-1]:
        #     isInside = np.logical_or(isInside, struct.isinside(sim.spins_d.get())) # str.isinside(sim.spins) return vector length num_spins & OR with isInside
        # self.assertTrue(isInside.all())
