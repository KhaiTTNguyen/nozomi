...
def plot_Dxy_across_compartment(folder_path, diff_time_limit, output_folder):
        ...
        if compartment is not None:
            ...
            try:
                with open(file_path, 'rb') as f:
                    ...
                    print(f"Loaded {filename}: {compartment} compartment")
    ...
    for compartment, data in averaged_data.items():
        ...
        print(f'{compartment} mean std Dxy: {np.mean(dxy_std_filtered):.4f}')
    ...
    plot_file_name = os.path.join(output_folder, f'RD_wrt_compartment_with_errorbars_difftime_limit{diff_time_limit}.png')
...
def plot_Dz_across_compartment(folder_path, diff_time_limit, output_folder):
    ...
    for compartment, data in averaged_results.items():
        ...
        print(f'{compartment} mean std Dz: {np.mean(dz_std_filtered):.4f}')
    ...
    plot_file_name = os.path.join(output_folder, f'AD_wrt_compartment_with_errorbars_difftime_limit{diff_time_limit}.png')
