...
def _plot_used_ogse_waveforms(waveform_records, output_dir):
    ...
    for rec in waveform_records:
        ...
        plt.savefig(
            os.path.join(
                output_dir,
                f"test_05_ogse_waveform_target_{rec['f_target_hz']:.0f}Hz_actual_{rec['f_actual_hz']:.0f}Hz.png",
            ),
            dpi=150,
        )
        plt.close(fig)
...
def _plot_final_spin_positions_3d_all(spins_xyz, lx, ly, lz, output_dir):
    ...
    ax.set_ylabel("y (um)")
    ax.set_zlabel("z (um)")
    ax.set_title("Test05: Final spin positions (3D, all spins) after OGSE simulation")
...
def test_radial_diffusion_signal_perpendicular_multi_cylinder_cosine_ogse_temporal_grid_5x5():
    """Test05: same OGSE setup as test04 with a 5x5 cylinder grid geometry."""
    ...
    summary_lines = ["Test05 OGSE Monte Carlo vs analytical signal summary", "=" * 52]
    ...
    ax_signal.set_title("Test05 radial: Monte Carlo vs analytical OGSE signal (0 to 500)")
    ax_signal.legend(frameon=True, fontsize=9)
    fig_signal.tight_layout()
    fig_signal.savefig(os.path.join(plot_dir, "test_05_ogse_radial_mc_vs_analytical_0_to_500.png"), dpi=150)
    ...
    fig_axial.savefig(os.path.join(plot_dir, "test_05_ogse_axial_mc_vs_analytical_0_to_500.png"), dpi=150)
    plt.close(fig_axial)
    ...
    ax_radial_extended.set_title("Test05 radial: Monte Carlo vs analytical OGSE signal (0 to 3000)")
    ax_radial_extended.legend(frameon=True, fontsize=9)
    fig_radial_extended.tight_layout()
    fig_radial_extended.savefig(
        os.path.join(plot_dir, "test_05_ogse_radial_mc_vs_analytical_0_to_3000.png"),
        dpi=150,
    )
