...
#     #     # sim_arena=last item in structures list --> seed in arena aka outsidesingle sphere
...
#     #     # make 2 overlapping spheres and put them in the middle of the geometry
#     #     # sx = np.array([6.1,3.9])
#     #     # sy = np.array([5,5])
#     #     # sz = np.array([5,5])
#     #     # sr = np.array([2,2])
...
#     #     nsphere = 50
#     #     sz = np.arange(-nsphere/2-2,nsphere/2+3)*Lx/nsphere
#     #     sy = np.sin(2*np.pi*np.arange(-2,nsphere+3)/nsphere)*0.3
#     #     sx = np.full(sz.shape, 0)
#     #     sr = np.full(sz.shape, 2)
#     #     # fiber = NodeList([Node() for i in range(len(sz))])
#     #     # fiber.x, fiber.y, fiber.z, fiber.radius = sx, sy, sz, sr
...
#     #     # 100000
#     #     dt = 0.01 # time step in ms
#     #     nt = 10 # total number of steps thru time
#     #     # nt = 10000 # total number of steps thru time
#     #     spins = int(1e4)
#     #     num_spins=spins
#     #     sim = ds3.DiffSim3d(sg3,spins)
...
#     #     isOutside = False
#     #     for structure in sg3.structures[:-1]:
#     #         isOutside = np.logical_or(isOutside, ~structure.isinside(sim.spins))
...
#     #         isOutside = False
#     #         for struct in sg3.structures[:-1]:
#     #             isOutside = np.logical_or(isOutside, ~struct.isinside(sim.spins_d.get()))
#     #         self.assertTrue(isOutside.all())
...
#     #         if n==9 or ~isOutside.all(): #and n%10==0:
#     #             print(n)
#     #             fig, ax = pl.subplots(subplot_kw={"projection": "3d"})
...
#     #         Dz_step[n] = np.sum(((sim.spins_d[2,:]-sim.spins0_d[2,:])**2).get())/spins/2/(n*dt)
#     #         diff_time[n] = n*dt
#     #     folder_name = './home/nguyt16@vanderbilt.edu/GPU_McDiffusionSim3D/diffsim3d/animation/ADC/debug_spin'
#     #     plot_ADC_vs_time(diff_time, Dx_step, Dy_step, Dz_step, folder_name , get_date_time())
