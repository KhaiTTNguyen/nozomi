...
def plot_Dxy_across_diameter(folder_path, diff_time_limit):
    ...
    ax.legend(handles=reordered_handles, labels=reordered_labels, fontsize=16)
    ...
    if compartment =='intra':
        ax.set_ylabel(r'$D_{\mathrm{i},\!\!\perp}$', fontsize=25) 
    elif compartment =='extra':
        ax.set_ylabel(r'$D_{\mathrm{e},\!\!\perp}$', fontsize=25) 
    
    ax.set_title(" $\kappa$="+str(kappa), fontsize=25)
...
def plot_Dz_across_diameter(folder_path, diff_time_limit):
    ...
    for diameter, data in averaged_results.items():
        ...
        time_mask = diff_time_mean <= diff_time_limit
        diff_time_filtered = diff_time_mean[time_mask]
        dz_mean_filtered = dz_mean[time_mask]
        dz_std_filtered = dz_std[time_mask]
        
        diameter_list.append(diameter)
        color_i = colors[diameter]
        
        # Plot mean line
        ax.semilogx(diff_time_filtered, dz_mean_filtered, color=color_i, 
                   label=str(diameter)+' µm', linewidth=2)
    ...
    ax.set_ylabel('AD (µm<math-inline>^2</math-inline>/ms)', fontsize=15) 
    ax.set_title(" Axial diffusivity vs. diffusion time at <math-inline>\kappa</math-inline>="+str(kappa), fontsize=17)
