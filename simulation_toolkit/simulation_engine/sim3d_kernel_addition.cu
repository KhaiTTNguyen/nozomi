...
extern "C"
{
    ...
    // __global__ void project_displacements(float* displacements, float* directions, 
    //                                      float* result, int Nspins, int Ndirr)
    // {
    //     // Calculate global indices
    //     int dir_idx = blockIdx.x * blockDim.x + threadIdx.x;
    //     int spin_idx = blockIdx.y * blockDim.y + threadIdx.y;
        
    //     if (dir_idx < Ndirr && spin_idx < Nspins) {
    //         float sum = 0.0f;
    //         // For each spatial dimension (3D)
    //         for (int i = 0; i < 3; i++) {
    //             sum += directions[i * Ndirr + dir_idx] * displacements[i * Nspins + spin_idx];
    //         }
    //         result[dir_idx * Nspins + spin_idx] = sum;
    //     }
    // }


    __global__ void computeKurtosis(float *spins, float *spins0, 
                               float *dx2_result, float *dy2_result, float *dz2_result,
        ...
        if (tid == 0) {
            atomicAdd(&dx2_result[step_idx], dx2_sum_block[0]/n_spins);
            atomicAdd(&dy2_result[step_idx], dy2_sum_block[0]/n_spins);
            atomicAdd(&dz2_result[step_idx], dz2_sum_block[0]/n_spins);
            atomicAdd(&dx4_result[step_idx], dx4_sum_block[0]/n_spins);
            atomicAdd(&dy4_result[step_idx], dy4_sum_block[0]/n_spins);
            atomicAdd(&dz4_result[step_idx], dz4_sum_block[0]/n_spins);
        }
    }
