...
class GeometricOptimization(object):
    ...
    def length_cost_function(self, positions, mask_starts_ends,torch_wrapped_ra):
        ...
        # a tensor the same size as row_distances, 
        # each entry holds the average inter-node spacing for the fiber that pair belongs to 
        # (i.e., start_end_distances[count] = end-to-start Euclidean distance ÷ number of gaps). 
        # This is the "expected" distance between consecutive nodes if they were evenly spaced.
        expected_spacing = torch.zeros_like(row_distances)
        count = 0
        for start_idx, end_idx in zip(start_indices, end_indices):
            expected_spacing[start_idx: end_idx] = start_end_distances[count]
            count=count+1
        # Creates a boolean mask that is False at positions where two consecutive nodes are both start/end boundary points
        space_mask = torch.logical_not(torch.logical_and(mask_starts_ends[:-1], mask_starts_ends[1:]))
        # scaled_row_distances = [ (actual inter-node distance - expected inter-node distance) ÷ expected inter-node distance ]^2
