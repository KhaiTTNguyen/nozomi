...
def plot_Dxy_across_diameters_for_K(organized_files, k_value, compartment, output_dir, diff_time_limit=120):
    ...
    # Formatting
    ax.set_xlabel(r'$t\;(\mathrm{ms})$', fontsize=21)
    ...
    # Set tick parameters for x-axis (log scale)
    ax.tick_params(axis='x', which='major', length=8, color='black', labelsize=13)
    ax.tick_params(axis='x', which='minor', length=4, color='black')
    
    # Set tick parameters for y-axis 
    ax.tick_params(axis='y', which='major', length=8, color='black', labelsize=13)
    
    ax.grid(True, alpha=0.8)
    print('diff_time_limit', diff_time_limit)
